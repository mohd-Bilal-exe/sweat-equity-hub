...
export default function Layout({ children, currentPageName }) {
  ...
  const filteredNav = () => {
    ...
    switch (user.user_type) {
      case 'talent':
        return navItems.filter(item => item.name !== 'Employer Dashboard');
      case 'employer':
        return navItems.filter(item => item.name !== 'Talent Dashboard');
      default:
        return navItems;
    }
  };
      ...
      <footer className="place-self-auto bg-gray-900 py-6 text-white">
        <div className="flex flex-col justify-between mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl">
          ...
          <div className="mt- pt-6 border-gray-800 border-t text-center">
            <p className="text-gray-400 text-sm">© 2025 sweatquity. All rights reserved.</p>
          </div>
        </div>
      </footer>
    </div>
